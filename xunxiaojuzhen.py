    ...
    with torch.no_grad():
        ...
        for i in range(0,X_test_RFE_vals.shape[0],batch_size):
            ...
            if len(outputs.data) > 0:
                ...
                # all_predicted.extend(predicted.cpu().numpy())  # 收集所有预测结果到列表
                n_samples += y.size(0)
                n_correct += (predicted == y).sum().item()
                # 收集所有预测结果到numpy数组
                if len(all_predicted) == 0:  # 第一次预测时直接赋值
                ...
                for j in range(y.size(0)):
                    confusion_mat[y[j].item(), predicted[j].item()] += 1
        ...
        # 确保y_test也是numpy数组，并且长度与all_predicted匹配
        assert len(y_test) == len(all_predicted), "预测结果数量与真实标签数量不匹配"
        # 计算性能指标
        precision = precision_score(y_test, all_predicted, average='weighted')
        recall = recall_score(y_test, all_predicted, average='weighted')
        f1 = f1_score(y_test, all_predicted, average='weighted')
        ...
        # 将精确率、召回率和F1分数添加到列表中
        accuracy_data.append({'Epoch': epoch + 1, 'Accuracy': acc, 'Precision': precision, 'Recall': recall, 'F1 Score': f1})
...
# 将DataFrame保存到CSV文件
accuracy_df.to_csv('accuracy——{4}.csv', index=False)

# print("准确率数据已保存到CSV文件中。")
        # 可视化混淆矩阵
plt.figure(figsize=(10, 8))
sns.heatmap(confusion_mat, annot=True, cmap='Blues', fmt='g', xticklabels=range(num_classes), yticklabels=range(num_classes))
